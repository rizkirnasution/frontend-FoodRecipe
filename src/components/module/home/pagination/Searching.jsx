    ...
    <>
      <div className="container">
        <nav className="navbar bg-tranparent mt-3">
          <div className="container d-flex ">
            ...
            <div className="d-flex search-container ">
              <form className="d-flex form-search" role="search" onSubmit={handleSearch1}>
                <input type="text" name="search" placeholder="search" className="form-control input-search " onChange={(e) => setSearch1(e.target.value)} />
            ...
            {recipes.length > 0 ? (
              recipes.map((item) => [
                <div className="col" key={item.id}>
                  <Card src={item.thumbnail} titleName={item.title} />
                </div>,
              ])
            ) : (
        ...
        <div className="d-flex flex-row my-5 justify-content-evenly">
          ...
          <button className="btn yellow btn-nav">&rarr;</button>
        </div>
      </div>
      <Footer />
    </>
